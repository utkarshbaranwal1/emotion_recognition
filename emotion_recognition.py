...
#below code allows to use male data only, comment it to use both male and female data
...
split_var = StratifiedShuffleSplit(1, test_size=0.25, random_state=4)
...
mcp_save = ModelCheckpoint('model\aug_noiseNpitchTuning_2class2_np.h5', save_best_only=True, monitor='val_loss', mode='min')
cnnhistory=model.fit(x_traincnn, y_train, batch_size=16, epochs=399, validation_data=(x_valcnn, y_val), callbacks=[mcp_save, lr_reduce])
...
loaded_model.load_weights("model\aug_noiseNpitchTuning_2class2_np.h5")
